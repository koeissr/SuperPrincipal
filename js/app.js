...
function renderSelectedTable() {
    ...
    const titleCount = document.getElementById("selectedTitleCount");
    if (titleCount) titleCount.textContent = `(${selectedCharacters.length} 人)`;
...
function startOptimizationFlow() {
    ...
    if (overviewBox) {
        overviewBox.classList.remove("d-none");
        overviewBox.innerHTML = `
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 fs-7">
                <div><i class="fa-solid fa-heart me-1 text-danger"></i>初始親密度：<b>${intimacy}%</b></div>
                <div><i class="fa-solid fa-flag-checkered me-1 text-primary"></i>交流上限：<b>${maxRoundsAllowed} 輪 (達 100%)</b></div>
                <div><i class="fa-solid fa-users me-1 text-info"></i>登場人數：<b>${characters.length} 人</b></div>
                <div><i class="fa-solid fa-bolt me-1 text-warning"></i>可用總戰力：<b>${totalScore.toLocaleString()}</b></div>
            </div>
        `;
    }
...
function exportCharExcelTemplate() {
    ...
    XLSX.writeFile(wb, "登場角色清單備份_" + new Date().toISOString().slice(0, 10) + ".xlsx");
}
...
function resetSelectedCharacters() {
    if (selectedCharacters.length > 0 && !confirm("確定要清空所有居民設定與已登場角色嗎？")) {
        return;
    }
    selectedCharacters = [];
    ...
    document.getElementById("mode1Badge").className = "badge bg-primary";
    document.getElementById("mode1Badge").textContent = "待運算";
    document.getElementById("mode2Badge").className = "badge bg-warning text-dark";
    document.getElementById("mode2Badge").textContent = "待運算";
}
...
function exportStrategyExcel() {
    ...
    XLSX.writeFile(wb, "古鎮居民交流雙模式戰術報表_" + new Date().toISOString().slice(0, 10) + ".xlsx");
}
