...
(function (root) {
    ...
    function solveDpExactMinimumOverkill(items, targetHP) {
        ...
        const dp = new Int32Array(maxTarget + 1).fill(-1);
        dp[0] = -2;
        const parent = new Array(maxTarget + 1);
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            for (let v = maxTarget - s; v >= 0; v--) {
                if (dp[v] !== -1) {
                    ...
                    if (dp[nextV] === -1) {
                        dp[nextV] = i;
                        parent[nextV] = { prevVal: v, itemIdx: i };
                    }
                }
            }
        }
        ...
        while (curr > 0 && parent[curr]) {
            const itemIdx = parent[curr].itemIdx;
            usedIndices.push(itemIdx);
            curr = parent[curr].prevVal;
        }
